...
public class PrefixImport<N extends Location, D extends State, W extends Weight> {
	...
	private abstract class IntersectionCallback{
		abstract void trigger(Transition<N, D> baseT,
				Transition<N, D> flowT);	
	}
	
	public PrefixImport(WeightedPAutomaton<N, D, W> autA, WeightedPAutomaton<N, D, W> autB, final Transition<N,D> t) {
		baseAutomaton = autA;
		flowAutomaton = autB;
		baseAutomaton.registerListener(new IntersectionListener(t.getStart(), t.getStart(), t.getLabel(), new IntersectionCallback() {

			@Override
			public void trigger(Transition<N, D> baseT,
					Transition<N, D> flowT) {
				//3.
				baseAutomaton.registerListener(new Import(t.getTarget(),flowT.getTarget()));
				baseAutomaton.registerListener(new IntersectionListenerNoLabel(t.getTarget(),flowT.getTarget(), new IntersectionCallback() {
