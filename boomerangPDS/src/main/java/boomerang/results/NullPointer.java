...
public class NullPointer {
	...
	public NullPointer(Statement sourceStatement, Val sourceVariable, Statement statement, Val variable, PAutomaton<Statement, INode<Val>> openingContext, PAutomaton<Statement, INode<Val>> closingContext) {
	...
	/**
	 * The variable that contains "null" and which provokes at {@link #getStatement() the statement} a NullPointerException.
	 ...
	 * 
	 * A null pointer can occur at three different types of statements:
	 * y = x.toString();
	 * or
	 * y = lengthof(x);
	 * or
	 ...
	 * 
	 * In the case of branching, there can be multiple call site contexts leading to a null pointer.
	 * Therefore, the opening context is represented as an automaton (or graph). The edges of the automaton are  
	 * labeled by the call sites, the nodes are labeled by variables or by variables at a context.
	 * For the example above, the automaton contains a transition with label foo(x)
	 * 
	 * 
	 ...
	 *  	x = returnNotNull(); //b2
	 *  }
	 * 	x.toString() //<- Variable x is null here when the program executed along branch b1
	 * }
	 * Object returnNull(){
	 * 	Object y = null;
	 *  return y;
	 ...
	 * 
	 * There can be multiple contexts leading to a null pointer.
	 * Therefore, the closing context is represented as an automaton (or graph). The edges of the automaton are  
	 * labeled by the call sites, the nodes are labeled by variables or by variables at a context.
	 * For the example above, the automaton contains a transition with label returnNull(). This indicates, that the null pointer
	 * only occurs along branch b1 but not b2. 
	 * 
	 * 
