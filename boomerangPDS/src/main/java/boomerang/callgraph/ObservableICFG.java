 ...
 *
 * @param <N>
 *            Nodes in the CFG, typically {@link Unit} or {@link Block}
 * @param <M>
...
public interface ObservableICFG<N, M> {
     ...
     * 
     * @param n
     *            The node for which to get the parent method
     */
    M getMethodOf(N n);
    ...
    /**
     * Returns all precomputed callers for a given method. Behaviour from {@link #addCalleeListener(CalleeListener)}
     * only differs when using dynamic call graph.
     */
    ...
    /**
     * Returns all start points of a given method. There may be more than one start point in case of a backward
     * analysis.
     */
    ...
    /**
     * Returns <code>true</code> if the given statement leads to a method return (exceptional or not). For backward
     * analyses may also be start statements.
     */
    boolean isExitStmt(N stmt);

    /**
     * Returns true is this is a method's start statement. For backward analyses those may also be return or throws
     * statements.
     */
    ...
    /**
     * Returns the list of parameter references used in the method's body. The list is as long as the number of
     * parameters declared in the associated method's signature. The list may have <code>null</code> entries for
     * parameters not referenced in the body. The returned list is of fixed size.
     */
     ...
     * @return True if there is a control flow path from the entry point of the program to the given statement,
     *         otherwise false
     */
    ...
    /**
     * Resets the call graph. Only affects the call graph if it was built demand-driven, otherwise graph will remain
     * unchanged. Demand-driven call graph will keep intraprocedual information, but reset start with an empty call
     * graph again.
     */
    void resetCallGraph();
